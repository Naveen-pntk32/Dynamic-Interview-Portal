...
const setdata = async () => {
    ...
    try{
        ...
        for (const course of courses) {
            // Compare category._id with the course.CategoryId
            if(!course.categoryId) continue;
            if (String(course.categoryId) === String(category._id)) {
                matchedCourses.push(course._id);
            }
        }

        // Add the matched coursesId field
        category.coursesId = matchedCourses;

        // Optionally update the document in DB if needed
        await category.save();

        console.log(`Updated category: ${category.name}`);
        console.log('Courses linked:', matchedCourses.length);
        }
    }
    catch(error){
